...
public class GeneradorReportePDF {
        ...
        public static String generarFacturaPDF(Factura factura) throws Exception {
                ...
                // Tamaño personalizado: 80mm x 180mm (227 pts x 510 pts)
                // Ideal para impresoras térmicas de 80mm de ancho
                PageSize pageSize = new PageSize(227, 510); // 80mm x 180mm
                PdfDocument pdf = new PdfDocument(writer);
                Document doc = new Document(pdf, pageSize);
                doc.setMargins(8, 8, 8, 8); // Márgenes muy reducidos
                // ---- ENCABEZADO EMPRESA ----
                Table cabecera = new Table(new float[] { 3, 2 })
                                .setWidth(UnitValue.createPercentValue(100));

                Cell celdaEmpresa = new Cell()
                                .add(new Paragraph(EMPRESA_NOMBRE).setFontSize(10).setBold()
                                                .setFontColor(COLOR_HEADER))
                                .add(new Paragraph(EMPRESA_NIT).setFontSize(7))
                                .add(new Paragraph(EMPRESA_DIR).setFontSize(7))
                                .add(new Paragraph(EMPRESA_TEL).setFontSize(7))
                                .setBorder(Border.NO_BORDER);
                cabecera.addCell(celdaEmpresa);

                Cell celdaFact = new Cell()
                                .add(new Paragraph("FACTURA ELECTRONICA").setFontSize(9).setBold()
                                                .setFontColor(COLOR_HEADER)
                                                .setTextAlignment(TextAlignment.RIGHT))
                                .add(new Paragraph("N°: " + factura.getNumero()).setFontSize(8)
                                                .setTextAlignment(TextAlignment.RIGHT).setBold())
                                .add(new Paragraph("Fecha: " + factura.getFecha().format(FMT_HORA))
                                                .setFontSize(7).setTextAlignment(TextAlignment.RIGHT))
                ...
                Table datosGrid = new Table(new float[] { 1, 1 })
                                .setWidth(UnitValue.createPercentValue(100))
                                .setMarginTop(2).setMarginBottom(2); // Márgenes pequeños
                ...
                doc.add(datosGrid);
                // Sin espacios en blanco entre secciones
                ...
                Table totales = new Table(new float[] { 4, 2 })
                                ...
                                .setMarginTop(2).setMarginBottom(2); // Márgenes muy pequeños
                ...
                doc.add(new Paragraph("Gracias por su compra. Conserve esta factura como soporte de su transaccion.")
                                .setFontSize(5).setItalic().setTextAlignment(TextAlignment.CENTER)
                                .setFontColor(ColorConstants.GRAY).setMargin(0).setMarginTop(2));
                doc.add(new Paragraph("Documento equivalente - Regimen simplificado")
                                .setFontSize(5).setTextAlignment(TextAlignment.CENTER)
                                .setFontColor(ColorConstants.GRAY).setMargin(0));
        ...
        private static Table lineaSeparadora() {
                ...
                // con borde superior coloreado — mismo efecto visual, compatible con todas las
                // versiones
        ...
        private static Cell bloqueInfo(String titulo, String... lineas) {
                Cell c = new Cell().setBorder(Border.NO_BORDER)
                                ...
                                .setPadding(2); // Padding muy reducido
                c.add(new Paragraph(titulo).setFontSize(6).setBold().setFontColor(COLOR_HEADER));
                for (String l : lineas)
                        c.add(new Paragraph(l).setFontSize(6).setFontColor(ColorConstants.DARK_GRAY)
                                        .setMargin(0).setMarginBottom(1)); // Sin márgenes, solo bottom pequeño
                return c;
        }

        private static Cell celdaHeader(String texto) {
                return new Cell()
                                .add(new Paragraph(texto).setFontSize(7).setBold().setFontColor(COLOR_TEXT_H))
                                .setBackgroundColor(COLOR_HEADER)
        ...
        private static Cell celdaFila(String texto, TextAlignment alin,
                ...
                Cell c = new Cell()
                                .add(new Paragraph(texto == null ? "" : texto).setFontSize(7).setMargin(0))
