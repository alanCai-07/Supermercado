...
public class ClienteFrame extends JFrame {
    ...
    private void construirUI() {
        ...
        g.gridx=0; g.gridy=0; g.gridwidth=2; root.add(titulo, g);

        String[] labels = {"NIT / CC:", "Nombre:", "Telefono:", "Email:", "Direccion:"};
        txtNit    = campo(); txtNombre = campo();
        txtTel    = campo(); txtEmail  = campo(); txtDir = campo();
        JTextField[] campos = {txtNit, txtNombre, txtTel, txtEmail, txtDir};

        for (int i = 0; i < labels.length; i++) {
            g.gridwidth=1; g.gridy = i+1; g.gridx=0; g.weightx=0.3;
            root.add(new JLabel(labels[i]), g);
            g.gridx=1; g.weightx=0.7; root.add(campos[i], g);
        }

        lblMsg = new JLabel(" ", SwingConstants.CENTER);
        lblMsg.setFont(new Font("Segoe UI", Font.ITALIC, 11));
        g.gridy=6; g.gridx=0; g.gridwidth=2; root.add(lblMsg, g);
        ...
        btnPanel.add(btnBuscar); btnPanel.add(btnGuardar);
        g.gridy=7; root.add(btnPanel, g);

        add(root);

        btnBuscar.addActionListener(e -> {
            String nit = txtNit.getText().trim();
            if (nit.isEmpty()) { lblMsg.setText("Ingrese el NIT."); return; }
            try {
