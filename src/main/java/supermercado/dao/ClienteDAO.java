...
public class ClienteDAO {
    ...
    public void guardar(Cliente c) throws SQLException {
        String sql = """
                INSERT INTO supermercado.clientes (nit, nombre, telefono, email, direccion)
                VALUES (?, ?, ?, ?, ?)
    ...
    public Cliente buscar(String nit) throws SQLException {
        String sql = "SELECT * FROM supermercado.clientes WHERE nit = ?";
        try (PreparedStatement ps = ConexionDB.getConexion().prepareStatement(sql)) {
            ps.setString(1, nit);
    ...
    public java.util.List<Cliente> buscarPorNombre(String texto) throws SQLException {
        ...
        String sql = "SELECT * FROM supermercado.clientes WHERE LOWER(nombre) LIKE LOWER(?) " +
